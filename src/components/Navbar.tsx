...
export default function Navbar() {
    ...
    return (
        ...
        className="fixed top-0 left-0 right-0 z-50 px-8 py-4 bg-transparent text-[#E6D5B7] font-body"
        >
        ...
        <AnimatePresence>
            {isMenuOpen && (
                ...
                className="absolute top-1/2 right-24 transform -translate-y-1/2 w-auto backdrop-blur-md bg-[#212844]/80 border border-[#EF8A76] rounded px-1 py-1 flex gap-1 z-40 text-xs shadow-xl"
                >
                {['HOME', 'ABOUT', 'PROJECTS'].map((label) => {
                    ...
                    return (
                        ...
                        >
                            <span className="relative z-10">{label}</span>
                            <span className="absolute inset-0 bg-[#EF8A76] scale-y-0 origin-bottom transition-transform duration-300 ease-in-out group-hover:scale-y-100 z-0"></span>
                        </button>
                    );
                })}
                </motion.div>
